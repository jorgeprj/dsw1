        ...
        <td>
            <select id="cnpj" name="cnpj">
                <c:forEach items="${listaEmpresas}" var="empresa">
                    <option value="${empresa.cnpj}" <c:if test="${empresa.cnpj == entrevista.cnpjEmpresa}">selected</c:if>>${empresa.nome} - ${empresa.cidade}</option>
                </c:forEach>
            </select>
        </td>
    </tr>
        ...
        <td>
            <select id="hora" name="hora">
                <script>
                    ...
                    var horaSelecionada = "${fn:substring(entrevista.dataHora, 11, 16)}"; 
